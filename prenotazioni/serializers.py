...
class BookingCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        is_available, _, errors = BookingService.check_resource_availability(
            risorsa_id=validated_data['risorsa'].id,
            inizio=validated_data['inizio'],
            fine=validated_data['fine'],
            quantita_richiesta=validated_data['quantita']
        )
        
        if not is_available:
            raise serializers.ValidationError(errors[0] if errors else "Risorsa non disponibile.")
...
class UserWithStatsSerializer(UtenteSerializer):
    ...
    def get_permissions(self, obj):
        ...
        return {
            'can_book': obj.can_book(),
            'can_modify_own_bookings': True,
            'can_modify_all_bookings': obj.is_admin(),
            'can_access_admin': obj.is_admin(),
            'can_manage_resources': obj.is_admin(),
