...
MIGRATION_DELETE_CODE="from django.db import connection; with connection.cursor() as c: c.execute(\"DELETE FROM django_migrations WHERE app = 'prenotazioni' AND name = '0001_initial'\"); print('Deleted prenotazioni migration record')"
python manage.py shell --settings=config.settings -c "$MIGRATION_DELETE_CODE"
